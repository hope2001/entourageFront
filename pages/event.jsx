...
import BottomBarEventContent from '@/content/Applications/Messenger/BottomBarEventContent';
...
function ApplicationsEventMessenger() {
...
const handleDrawerToggle = () => {
  ...
  useEffect(() => {
    ...
    fromHistory.map((item) =>{
      ...
      list.push({who: "human", data: item.query}, {who: "ia", data: JSON.parse(item.answer), id: item.id})
...
/*/}
        <ChatWindow style={{backgroundColor:"#F8EFEA"}}>
          ...
          {/* <Divider /> */}
          <BottomBarEventContent chat_id={chat_id} setchat_id={setchat_id} converse={converse} setConverse={setConverse} chatLoad={chatLoad} setchatLoad={setchatLoad} response={response} setResponse={setResponse} error={error} setError={setError} inter={inter} setinter={setinter} setquery={setquery}  />
        </ChatWindow>
      </RootWrapper>
    </div>
...
ApplicationsEventMessenger.getLayout = (page) => (
  <SidebarLayout>{page}</SidebarLayout>
);

export default ApplicationsEventMessenger;
